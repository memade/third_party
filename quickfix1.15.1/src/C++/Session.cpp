...
#define LOGEX( method ) try { method; } catch( std::exception& e ) \
  { m_state.onEvent( e.what() ); }
...
#if 1//!@ By Martell 20210311
		void* pContext = nullptr;
		m_application.onCreate(m_sessionID, &pContext);
		if (pContext)
		{
			m_pContext = reinterpret_cast<IContext*>(pContext);
			m_pContext->m_pSession = this;
		}
#endif
	...
	{
#if HAVE_PACKEY_COMPRESS
		if (FIX::MsgType_SourceRawData == msgType)
		{
			FIX::RawData rawData{ "" };
			FIX::RawDataLength rawDataLength{ 0 };
			FIX::SourceDataLength sourceDataLength{ 0 };
			if (msg.isSetField(FIX::FIELD::RawData)) msg.getField(rawData);
			if (msg.isSetField(FIX::FIELD::RawDataLength)) msg.getField(rawDataLength);
			if (msg.isSetField(FIX::FIELD::SourceDataLength)) msg.getField(sourceDataLength);
			if (sourceDataLength.getValue() > 0 && sourceDataLength.getValue() != rawDataLength.getValue())
			{
				std::string unzipOut;
				...
				if (Z_OK == shared::Zip::zipUnCompress(rawData.getValue(), nSource, unzipOut))
				{
					...
					nowMsg.setField(FIX::RawDataLength(static_cast<unsigned int>(unzipOut.size())));
					nowMsg.setField(FIX::SourceDataLength(static_cast<unsigned int>(unzipOut.size())));

					if (Message::isAdminMsgType(msgType))
					{
						m_application.fromAdmin(nowMsg, m_sessionID);
					}
					else
					{
						m_application.fromApp(nowMsg, m_sessionID);
					}
					return;
				}
			}
	...
	{
#if HAVE_PACKEY_COMPRESS
		if (FIX::MsgType_SourceRawData == message.getHeader().getField(FIX::FIELD::MsgType))
		{
			FIX::RawData rawData{ "" };
			if (message.isSetField(FIX::FIELD::RawData)) rawData = message.getField(FIX::FIELD::RawData);
	...
	size_t Session::PushToAllTargetSessions(Message& message)
	{
		size_t result = 0;
		Locker locker(s_mutex);
		for (const auto& session : s_sessions)
			if (session.second)
				if (session.second->SendToTarget(message))
					++result;
		return result;
	}
	void Session::iterateSession(const std::function<void(Session*)>& iterate_cb)
	{
		Locker locker(s_mutex);
		for (const auto& session : s_sessions)
			if (session.second)
				iterate_cb(session.second);
	}
	void Session::iterateContext(const std::function<void(IContext*)>& iterate_cb)
	{
		Locker locker(s_mutex);
		for (const auto& session : s_sessions)
			if (session.second && session.second->Context() && session.second->Context()->Ready())
				iterate_cb(session.second->Context());
	}
	IContext* Session::lookupContext(const unsigned __int64& contextID)
	{
		Locker locker(s_mutex);
		for (const auto& session : s_sessions)
			if (session.second->m_pContext)
				if (session.second->m_pContext->ContextID() == contextID)
					return session.second->m_pContext;
		return nullptr;
	}
	IContext* Session::lookupContext(const SessionID& sessionID)
	{
		Locker locker(s_mutex);
		Sessions::iterator find = s_sessions.find(sessionID);
		if (find != s_sessions.end())
			if (find->second && find->second->Context())
				return find->second->Context();
		return nullptr;
	}
	size_t Session::PushToAllTargetContexts(Message& message)
	{
		size_t result = 0;
		Locker locker(s_mutex);
		for (const auto& session : s_sessions)
			if (session.second && session.second->m_pContext && session.second->m_pContext->Ready())
				if (session.second->SendToTarget(message))
					++result;
		return result;
	}
	bool Session::SendToTarget(const SessionID& sessionID, Message& message)
	{
		Session* pSession = lookupSession(sessionID);
		if (!pSession)
			return false;
		return pSession->SendToTarget(message);
	}
#endif
	...
	void Session::removeSession(Session& s)
	{
		...
		if (s.m_pContext)
		{
			s.m_pContext->Release();
			s.m_pContext = nullptr;
		}
#endif
...
#if 1//!@ By Martell 20210311
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	IContext* Session::Context() const
	{
		return m_pContext;
	}
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	IContext::IContext(const SessionID& sessionID) :
		m_SessionID(sessionID),
		m_ContextID(::strtoull(sessionID.getTargetCompID().getValue().c_str(), nullptr, 16))
	{
	}
	IContext::~IContext()
	{
	}
	bool IContext::Ready() const
	{
		if (!m_pSession)
			return false;
		return m_pSession->isLoggedOn();
	}
	bool IContext::Send(FIX::Message& message)
	{
		if (!m_pSession)
			return false;
		return m_pSession->SendToTarget(message);
	}
	Session* IContext::SessionGet() const
	{
		return m_pSession;
	}
	const SessionID& IContext::SessionIDGet() const
	{
		return m_SessionID;
	}
	const unsigned long long& IContext::ContextID() const
	{
		return m_ContextID;
	}
#endif
